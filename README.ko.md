...
그래서 Perturb는 특정 항목을 찍어서 검사하지 않고 **도메인 전체를 열거**합니다. 마운트된 모든 파일시스템을 전부 훑고, 프로퍼티는 값과 함께 변경 카운터(serial)까지 전부 뜨고, 볼 수 있는 모든 프로세스의 `mountinfo`를 뜨고, 커널 집계 숫자와 ART 컴파일 상태를 뜹니다.
...
| 전이 | 레이어 | 추가 | 삭제 | 변경 |
|---|---|---:|---:|---:|
| **S0 → S1** (Magisk) | `fs` | 1,205 | 0 | 88 |
| | `mount` | 47 | 2 | 3,604 |
| | `prop` | 18 | 0 | 121 |
| | `selinux` | 0 | 0 | 1 |
| | `aggregate` | 0 | 0 | 9 |
| **S1 → S2b** (ReZygisk) | `maps` | 3 | 0 | 12 |
| | `fd` | 2 | 0 | 0 |
| **S3 → S4** (은닉 적용) | `mount` | 0 | **44** | 0 |

마지막 줄이 이 프로젝트가 노리는 지점입니다 — 은닉을 켜면 **삭제만 44건** 일어납니다. 추가된 항목만 보는 도구는 이 단계를 통째로 놓칩니다.
...
`clean` 대 `rooted` 두 상태만 비교하면 서로 다른 원인들이 뒤섞입니다. Perturb는 **바로 옆 단계끼리만 diff를 떠서** 각 단계가 한 가지 원인만 분리하게 합니다.
